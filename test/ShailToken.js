...
contract('ShailToken', function(accounts) {
  var tokenInstance;

   it('initializes the contract with the correct values', function() {
    return ShailToken.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, 'Shail Token', 'has correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'SHAIL', 'has correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'Shail Token v1.0', 'Has correct standard');
    });
   })

  it('allocates sets the inital supply upon deployment', function() {
    return ShailToken.deployed().then(function(instance) {
      tokenInstance = instance;
